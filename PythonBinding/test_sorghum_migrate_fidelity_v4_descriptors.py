...
class SorghumFidelityMigrationTest(unittest.TestCase):
    def test_stage_widths_follow_early_reference_and_mature_authored_values(self) -> None:
        self.assertEqual(STAGES["GrowthStage01"], (0.0120, 0.0800, 0.00030, 0.00015))
        self.assertEqual(STAGES["GrowthStage02"], (0.0135, 0.0950, 0.00035, 0.00020))
        self.assertEqual(STAGES["GrowthStage03"], (0.0150, 0.1100, 0.00045, 0.00030))
        self.assertEqual(STAGES["GrowthStage04"][1], 0.1260)
        self.assertEqual(STAGES["GrowthStage05"][1], 0.1260)
    ...
    def test_migration_is_idempotent_and_materializes_leaf_mechanics(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            for cultivar in LEAF_PROFILES:
                ...
                self.assertIn("max_value: 0.08", field_block(first, "leaf_blade_max_width"))
                self.assertIn("- [0, 0.228]", field_block(first, "leaf_blade_max_width"))
                self.assertIn(
                    f"max_value: {LEAF_MAX_LENGTH_M['GrowthStage01'][cultivar]:.8g}",
                    field_block(first, "leaf_blade_length"),
                )
                self.assertIn("max_value: 0.0003", field_block(first, "leaf_blade_thickness"))
                self.assertIn("tangent_: false", field_block(first, "leaf_gravity_droop_age_response"))
                self.assertIn("mean: 0.02", field_block(first, "leaf_gravity_droop_compliance"))
                self.assertIn("mean: 0.008", field_block(first, "leaf_centerline_waviness_fraction"))
                self.assertIn("mean: 0.018", field_block(first, "leaf_static_wind_deflection_fraction"))
                self.assertIn("mean: 30", field_block(first, "leaf_axial_twist_max_degrees"))
                self.assertIn("mean: 0.35", field_block(first, "leaf_axial_twist_frequency_ratio_min"))
                self.assertIn("mean: 0.5", field_block(first, "leaf_axial_twist_frequency_ratio_max"))
                self.assertNotRegex(first, r"(?m)^leaf_static_twist_degrees:")
                self.assertIn("min_value: 0.02", field_block(first, "leaf_flexural_stiffness_along_leaf"))
                self.assertIn("mean: 0", field_block(first, "leaf_damage_severity"))
                self.assertIn("mean: 1.4", field_block(first, "leaf_sheath_cross_section_ratio"))
                ...
                self.assertIn(f"mean: {TARGET_GDD_BY_STAGE['GrowthStage01']:.8g}", field_block(first, "target_gdd"))
                self.assertIn(f"mean: {MATURITY_GDD:.8g}", field_block(first, "maturity_gdd"))
                self.assertIn(
    ...
    def test_migration_materializes_mechanics_missing_from_legacy_descriptor(self) -> None:
        required = (
            "leaf_waviness_width_fraction",
            "leaf_waviness_wavelength_m",
            "leaf_centerline_waviness_fraction",
    ...
    def test_cultivars_keep_distinct_rest_architecture_and_vertical_newest_leaf(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("max_value: 42", field_block(migrated["BTX"], "leaf_insertion_angle"))
            self.assertIn("max_value: 70", field_block(migrated["Pawaga"], "leaf_insertion_angle"))
            self.assertIn("max_value: 150", field_block(migrated["BTX"], "leaf_bending"))
            self.assertIn("max_value: 100", field_block(migrated["Pawaga"], "leaf_bending"))
            ...
            self.assertIn("- [0.5, 0.015]", btx_bend_profile)
            self.assertIn("- [0.7, 0.18]", btx_bend_profile)
            self.assertIn("- [0.88, 0.6]", btx_bend_profile)
            pawaga_bend_profile = field_block(migrated["Pawaga"], "bending_along_leaf")
            self.assertIn("- [0.5, 0.026]", pawaga_bend_profile)
            self.assertIn("- [0.7, 0.2]", pawaga_bend_profile)
            self.assertIn("- [0.88, 0.62]", pawaga_bend_profile)
    ...
    def test_posture_update_preserves_calibrated_growth_timing(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("- [0.45, 0.05]", field_block(text, "leaf_bending_development_curve"))
            self.assertIn("- [0.45, 0]", field_block(text, "leaf_gravity_droop_age_response"))

    def test_manual_descriptor_can_keep_legacy_snapshot_finalization(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "BTX.sorghumls"
            path.write_bytes((MANUAL / path.name).read_bytes())
            migrate(path, (0.0210, 0.1260, 0.00050, 0.00040), "GrowthStage04", finalize_snapshot=True)
            self.assertIn("finalize_snapshot_morphology: true", path.read_text(encoding="utf-8"))
