...
def checkpoint_fingerprint(
    ...
    if args.video:
        inputs["video_driver"] = Path(__file__).with_name(
            "sorghum_4x10_illumination_video.py"
        )
...
def sensor_rows_with_workers_locked(
    ...
    for date in args.dates:
        ...
        while True:
            ...
            capture_worker_completed = (
                getattr(args, "video", False)
                and next_replicate > previous_replicate
                and (next_replicate < args.replicates or sensor_csv.is_file())
            )
            # RayTracerCamera DLL teardown can fault after durable output, as in the mobile capture workers.
            if capture_worker_completed:
                validate_staged_prefix(
                    video_staging_root(args.checkpoint_dir),
                    date,
                    next_replicate,
                    args.probes_per_panel,
            ...
            if returncode != 0:
                print(
                    f"{date}: capture worker teardown warning={returncode}; durable outputs validated",
                    flush=True,
                )
            if next_replicate == args.replicates:
                break
...
{f'- `visualizations/{VIDEO_NAME}`: {VIDEO_SPEC.fps} FPS scene and running-mean PARBAR video; its JSON manifest records layout, scale, duration, and checksum.' if args.video else ''}
...
Before tracing rays, each scene is prepared so `Ground Mesh` uses the PBR soil material through a `MeshRenderer`. Each date scene also swaps BTX and Pawaga plants onto the calibrated date/cultivar descriptors, then regenerates all 40 rooted plants for every replicate.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--config", default="RelWithDebInfo", help=argparse.SUPPRESS)
    parser.add_argument(
        "--runtime-package-dir",
        type=Path,
        default=repo_root
    ...
    parser.add_argument(
        "--calibrated-descriptor-root",
        type=Path,
        default=GENERATED_DESCRIPTOR_ROOT,
        help=argparse.SUPPRESS,
    )
    parser.add_argument(
        "--reference-scene", default=REFERENCE_4X10_SCENE, help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Camera samples per pixel; independent of scientific PARBAR samples.",
    )
    parser.add_argument(
        "--video-bounces",
        type=int,
        default=2,
        help="Camera ray bounces; independent of scientific PARBAR bounces.",
    )
    parser.add_argument("--seed", type=int, default=0, help=argparse.SUPPRESS)
    parser.add_argument(
        "--geometry-seed", type=int, default=2_000_000, help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--geometry-seed-stride", type=int, default=1000, help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--push-normal-distance", type=float, default=0.001, help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--middle-panel-height-fraction",
        type=float,
    ...
    parser.add_argument(
        "--max-wait-frames", type=int, default=30000, help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--after-panel-move-frames", type=int, default=2, help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--checkpoint-dir", type=Path, default=None, help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--checkpoint-interval", type=int, default=100, help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--progress-interval", type=int, default=10, help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--engine-batch-size",
        type=int,
...
def run_parent_campaign(
    ...
    if video_paths:
        index_rows.extend(
            [
                {
                    "file": str(video_paths[0].relative_to(args.output_dir).as_posix()),
                    "row_count": len(args.dates) * VIDEO_SPEC.frames_per_date,
                    "description": "Scene realizations above six running-mean PARBAR probe panels.",
                },
                {
                    "file": str(video_paths[1].relative_to(args.output_dir).as_posix()),
                    "row_count": 1,
                    "description": "Campaign video layout, timing, scale, and SHA-256 metadata.",
                },
            ]
        )
    write_csv(
...
def main() -> None:
    ...
    if args.video:
        if args.video_samples <= 0 or args.video_bounces < 0:
            raise ValueError(
                "--video-samples must be positive and --video-bounces must be nonnegative"
            )
