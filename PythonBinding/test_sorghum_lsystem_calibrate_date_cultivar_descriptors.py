...
from sorghum_validate_2026_morphology import summarize_rank_profile
...
class SorghumPhenotypeSummaryTests(unittest.TestCase):
    ...
    def test_rank_profile_keeps_target_dimensions_separate_from_realized_growth(self) -> None:
        leaves = [
            SimpleNamespace(
                axis_id=0,
                alive=True,
                normalized_rank=0.0,
                blade_length_m=0.2,
                target_blade_length_m=0.4,
                blade_width_m=0.02,
                target_blade_width_m=0.04,
                growth_progress=0.5,
            ),
            SimpleNamespace(
                axis_id=0,
                ...
                normalized_rank=0.8,
                blade_length_m=0.5,
                target_blade_length_m=0.5,
                blade_width_m=0.08,
                target_blade_width_m=0.08,
                growth_progress=1.0,
            ),
        ]

        rows = {row["normalized_rank"]: row for row in summarize_rank_profile([SimpleNamespace(leaves=leaves)])}

        self.assertAlmostEqual(rows[0.0]["model_realized_length_fraction"], 0.4)
        self.assertAlmostEqual(rows[0.0]["model_target_length_fraction"], 0.8)
        self.assertAlmostEqual(rows[0.0]["model_realized_width_fraction"], 0.25)
        self.assertAlmostEqual(rows[0.0]["model_target_width_fraction"], 0.5)
