...
std::vector<std::vector<glm::vec3>> CollectManualTwoByTenBlock(const std::shared_ptr<Scene>& scene,
  ...
  for (auto& row : rows) {
    std::sort(row.begin(), row.end(), [](const glm::vec3& left, const glm::vec3& right) { return left.x < right.x; });
    const auto [min_z, max_z] = std::minmax_element(row.begin(), row.end(), [](const glm::vec3& left,
...
void RenameEntityHierarchyPrefix(const std::shared_ptr<Scene>& scene, const Entity& root,
...
void PyDigitalAgriculture::Initialize(pybind11::module& m) {
  ...
  m.def("SetSorghumLsLeafWidthScale", &SetSorghumLsLeafWidthScale, py::arg("leaf_width_scale"),
        py::arg("regenerate_geometry") = true);
  m.def("ConfigureSorghumLsLeafMeshQuality", &ConfigureSorghumLsLeafMeshQuality,
        py::arg("vertical_subdivision_length"), py::arg("horizontal_subdivision_step"),
        py::arg("bottom_face") = true, py::arg("enable_leaf_sheath") = true,
  ...
  py::class_<LSystemPlantSceneMetadataRecord>(m, "LSystemPlantSceneMetadataRecord")
      ...
      .def_readonly("geometry_snapshot_schema_version",
                    &LSystemPlantSceneMetadataRecord::geometry_snapshot_schema_version)
      .def_readonly("geometry_snapshot_version", &LSystemPlantSceneMetadataRecord::geometry_snapshot_version)
      .def_readonly("geometry_snapshot_organ_count", &LSystemPlantSceneMetadataRecord::geometry_snapshot_organ_count)
      .def_readonly("leaf_vertex_count", &LSystemPlantSceneMetadataRecord::leaf_vertex_count)
...
#  ifdef CUDA_MODULE_SERVICE
  if (const auto& ray_tracer = CudaModule::GetRayTracer()) {
    return {ray_tracer->gas_build_count, ray_tracer->gas_update_count, ray_tracer->ias_build_count,
            ray_tracer->ias_update_count};
  }
#  endif
  return {};
}
...
size_t PyDigitalAgriculture::GrowSorghumLsPlantsToAdulthood(const int seed_base, const std::string& cultivar_filter,
                                                            const bool reuse_geometry_entities,
  ...
  if (reuse_geometry_entities) {
    ...
    for (const auto& plant : plants) {
      plant->GenerateGeometryEntities(true);
    }
  }
  if (!plants.empty()) {
    TransformGraph::CalculateTransformGraphs(scene);
...
size_t PyDigitalAgriculture::ConfigureSorghumLsPlantingGrid(const std::vector<std::string>& row_genotypes,
  ...
  for (const auto& entity : scene->UnsafeGetAllEntities()) {
    ...
    if (scene->IsEntityValid(parent) && std::find(old_parents.begin(), old_parents.end(), parent) == old_parents.end()) {
  ...
  for (const auto& parent : old_parents) {
    ...
    if (scene->IsEntityValid(parent) && scene->GetChildren(parent).empty() &&
        parent_name.size() >= 6 && parent_name.compare(parent_name.size() - 6, 6, "_Field") == 0) {
  ...
  for (size_t row = 0; row < row_genotypes.size(); ++row) {
    for (uint32_t column = 0; column < columns; ++column) {
      const std::string name = row_genotypes[row] + "_LSystem_R" + std::to_string(row) + "_C" +
      ...
      transform.SetPosition(glm::vec3(center_x_m + (static_cast<float>(column) - center_column) * column_spacing_m,
                                      0.0f,
                                      center_z_m + (static_cast<float>(row) - center_row) * row_spacing_m));
...
size_t PyDigitalAgriculture::ConfigureSorghumLsReplicatedSixByTenProfile(
  ...
  const glm::vec3 block_offset = glm::vec3(MeanPosition(second_block) - MeanPosition(first_block)) *
  ...
  for (size_t block = 0; block < blocks.size(); ++block) {
    for (size_t block_row = 0; block_row < blocks[block].size(); ++block_row) {
      ...
      for (size_t column = 0; column < blocks[block][block_row].size(); ++column) {
        const Entity marker = scene->CreateEntity(block_genotypes[block] + "_LSystem_R" + std::to_string(row) +
                                                  "_C" + std::to_string(column));
        auto transform = scene->GetDataComponent<Transform>(marker);
...
std::vector<LSystemPlantSceneMetadataRecord> PyDigitalAgriculture::GetSorghumLsPlantSceneMetadata(
  ...
  for (const auto& plant : *sorghum_entities_ptr) {
    ...
    if (const auto sorghum = scene->GetOrSetPrivateComponent<SorghumLS>(plant).lock()) {
      ...
      record.primary_tiller_count = architecture.primary_tiller_count;
      record.axes = architecture.axes;
      if (const auto& snapshot = sorghum->GetGeometrySnapshot()) {
        record.geometry_snapshot_schema_version = snapshot->schema_version;
        record.geometry_snapshot_version = snapshot->geometry_version;
        record.geometry_snapshot_organ_count = static_cast<uint32_t>(snapshot->organ_ranges.size());
        record.leaf_vertex_count = static_cast<uint32_t>(snapshot->leaf_vertices.size());
